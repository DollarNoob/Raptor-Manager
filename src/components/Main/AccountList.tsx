		...
		<div style={style}>
			...
			{store.profiles.sort(filters[filter.filter]).map((profile, i) => (
				<Account
					...
					profile={profile}
					state={
						store.states.find((state) => state.profileId === profile.id) ?? { profileId: profile.id, connected: false, pid: null, client: null, port: null }
					}
					onClick={() =>
						store.setSelectedIndex(store.selectedIndex === i ? null : i)
					}
				/>
			))}
		</div>
