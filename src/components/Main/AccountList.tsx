...
export default function AccountList({}: Props) {
  ...
  const filters: ((a: IProfile, b: IProfile) => number)[] = [
    ...
    (a, b) => (a.name ?? "Profile: " + a.id).localeCompare((b.name ?? "Profile: " + b.id)), // A to Z (Profile Name)
    (a, b) => (b.name ?? "Profile: " + b.id).localeCompare((a.name ?? "Profile: " + a.id)) // Z to A (Profile Name)
  ];
      ...
      <AccountManager/>
      {
        store.profiles.sort(filters[filter.filter]).map((profile, i) => (
          <Account
            ...
            profile={ profile }
            state={ store.states.find(state => state.profileId === profile.id)! }
            onClick={ () => store.setSelectedIndex(store.selectedIndex === i ? null : i) }
          />
        ))
      }
    </div>
