...
export default function Settings({}: Props) {
  ...
  async function onInstall(client: string) {
    const removed = await removeClient(client)
    ...
    const installed = await installClient(client)
      .catch((err) => new Error(err));
  ...
  async function switchDecompiler() {
    ...
    const newDecompiler = decompilerList[(index+1) % decompilerList.length];
  ...
  const robloxInstallation = config.config.clients.find(client => client.name === "Vanilla");
  const macsploitInstallation = config.config.clients.find(client => client.name === "MacSploit");
  const hydrogenInstallation = config.config.clients.find(client => client.name === "Hydrogen");

  return (<>
    <main style={ style }>
      <div style={ clientContainerStyle }>
        <Client installation={ robloxInstallation } version={ version.roblox } thumbnail="/roblox.svg" onInstall={ onInstall } onRemove={ onRemove }>
          Vanilla
        </Client>
        <Client installation={ macsploitInstallation } version={ version.macsploit } thumbnail="/macsploit.png" onInstall={ onInstall } onRemove={ onRemove }>
          MacSploit
        </Client>
        <Client installation={ hydrogenInstallation } version={ version.hydrogen } thumbnail="/hydrogen.png" onInstall={ onInstall } onRemove={ onRemove }>
          Hydrogen
        </Client>
      </div>
      <Button color="blue" onClick={ switchDecompiler }>{ config.config.decompiler.charAt(0).toUpperCase() + config.config.decompiler.slice(1) }</Button>
    </main>
  </>);
}
