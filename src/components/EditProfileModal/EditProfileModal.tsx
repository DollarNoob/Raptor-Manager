...
import { IProfile, IRobloxProfile } from "../../types";
...
export default function EditProfileModal({ profile, destruct }: Props) {
  ...
  const cookieRef = useRef<[string, React.Dispatch<React.SetStateAction<string>>]>(null);
  const nameRef = useRef<[string, React.Dispatch<React.SetStateAction<string>>]>(null);
  const noteRef = useRef<[string, React.Dispatch<React.SetStateAction<string>>]>(null);
  ...
  async function importCookies() {
    const session = await invoke("import_cookies")
      .catch((err) => new Error(err));
    if (session instanceof Error) {
      if (session.message === "a webview with label `login` already exists") {
  ...
  async function update() {
    ...
    const robloxProfile = await invoke<IRobloxProfile>("get_roblox_profile", { cookie: cookieRef.current[0] })
      .catch((err) => new Error(err));
    if (robloxProfile instanceof Error) {
      ...
      modal.add({
        ...
        text: robloxProfile.message === "401 Unauthorized" ? "Provided cookie is invalid. Try using import to load a cookie." : robloxProfile.message,
        buttons: [
          {
            text: "Okay",
    ...
    const robloxThumbnail = await invoke<string>("get_roblox_thumbnail", { userId: robloxProfile.id })
      .catch((err) => new Error(err));
  ...
  return (<>
    <div style={ style }>
      ...
      <Input ref={ cookieRef } placeholder=".ROBLOSECURITY" defaultValue={ profile.cookie }/>
      <Text>Please input a profile name. (optional)</Text>
      <Input ref={ nameRef } placeholder="Profile: Nexus42" defaultValue={ profile.name ?? "" }/>
      <Text>Please input a note. (optional)</Text>
      <Input ref={ noteRef } placeholder="This is the default note." defaultValue={ profile.note }/>
