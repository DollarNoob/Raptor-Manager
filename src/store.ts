...
import { IConfig, IHydrogenVersion, IMacsploitVersion, IModal, IProfile, IRobloxVersion, IState } from "./types";
...
export const useStore = create<StoreState>()((set) => ({
  ...
  updateProfile: (newProfile) => set((state) => ({ profiles: state.profiles.map(p => p.id === newProfile.id ? newProfile : p) })),
  removeProfile: (id) => set((state) => ({
    profiles: state.profiles.filter(profile => profile.id !== id),
    states: state.states.filter(state => state.profileId !== id)
  })),
  updateState: (newState) => set((state) => ({ states: state.states.map(s => s.profileId === newState.profileId ? newState : s) })),
  selectedIndex: null,
...
export const useModalStore = create<ModalState>()((set) => ({
  ...
  remove: (id) => set((state) => ({ modals: state.modals.filter(modal => modal.id !== id) }))
}));
