...
import { IConfig, IRobloxVersion, IMacsploitVersion, IHydrogenVersion, IState, IProfile } from "./types";
import { useConfigStore, useModalStore, useStore, useVersionStore } from "./store";
...
export async function launchClient(client: string, profileId: string, cookie: string) {
  const unlocked = await invoke<number>("unlock_keychain", { profileId })
    .catch((err) => new Error(err));
  if (unlocked instanceof Error) throw unlocked;

  if (unlocked === 50) throw new Error("Keychain was not found. It seems like your profile is corrupted, please remove the profile and create it again.");
  else if (unlocked !== 0) throw new Error(`Could not unlock keychain with code ${unlocked}.`);

  const written = await invoke<null>("write_cookies", { profileId, cookie })
    .catch((err) => new Error(err));
  if (written instanceof Error) throw written;

  const modified = await invoke<null>("modify_bundle_identifier", { client, profileId })
    .catch((err) => new Error(err));
  if (modified instanceof Error) throw modified;

  const launched = await invoke<IState>("launch_client", { client, profileId })
    .catch((err) => new Error(err));
...
export async function stopClient(pid: number) {
  const stopped = await invoke("stop_client", { pid })
    .catch((err) => new Error(err));
  if (stopped instanceof Error) throw stopped;
  if (!stopped) throw new Error("Client could not be stopped due to an unknown error.");
}

export async function readConfig() {
  const cfg = await invoke<IConfig>("read_config")
    .catch((err) => new Error(err));
  ...
  if (cfg instanceof Error) {
    ...
    modal.add({
      ...
      buttons: [
        ...
        {
          text: "Yes",
          onClick: () => modal.remove(id) ?? writeConfig({ client: null, clients: [], decompiler: "medal" })
        }
      ]
    });
    return false;
...
export async function writeConfig(config: IConfig) {
  ...
  const cfg = await invoke<IConfig>("write_config", { config })
    .catch((err) => new Error(err));
...
export async function readProfiles() {
  const profiles = await invoke<IProfile[]>("read_profiles")
    .catch((err) => new Error(err));
...
export async function fetchClientVersions() {
  ...
  const robloxPromise = invoke<IRobloxVersion>("get_roblox_version")
    .catch((err) => new Error(err));
  const macsploitPromise = invoke<IMacsploitVersion>("get_macsploit_version")
    .catch((err) => new Error(err));
  const hydrogenPromise = invoke<IHydrogenVersion>("get_hydrogen_version")
    .catch((err) => new Error(err));

  const [ roblox, macsploit, hydrogen ] = await Promise.all([ robloxPromise, macsploitPromise, hydrogenPromise ]);
...
export async function installClient(client: string) {
  ...
  if (client === "Vanilla") {
    if (!clientVersion) throw new Error("Roblox version is not fetched yet.");
    if (!dylibVersion) throw new Error("Dylib version is not fetched yet.");
  } else if (client === "MacSploit") {
    if (!version.macsploit.clientVersionUpload || !version.macsploit.relVersion)
    ...
    if (!version.hydrogen.macos.roblox_version || !version.hydrogen.macos.exploit_version)
  ...
  const installed = await invoke("install_client", { client, version: clientVersion })
...
export async function removeClient(client: string) {
  const installed = await invoke("remove_client", { client })
    .catch((err: string) => new Error(err));
  if (installed instanceof Error) throw installed;
...
export async function addProfile(profile: IProfile) {
  ...
  const added = await invoke<void>("write_profiles", { profiles: [ ...store.profiles, profile ] })
    .catch((err: string) => new Error(err));
  ...
  const envCreated = await invoke<void>("create_environment", { id: profile.id })
    .catch((err) => new Error(err));
  ...
  const keychain = await invoke<number>("create_keychain", { profileId: profile.id })
    .catch((err) => new Error(err));
...
export async function updateProfile(profile: IProfile) {
  ...
  const newProfiles = store.profiles.map(p => p.id === profile.id ? profile : p);
  const updated = await invoke<void>("write_profiles", { profiles: newProfiles })
    .catch((err: string) => new Error(err));
...
export async function removeProfile(id: string) {
  const store = useStore.getState();
  const removed = await invoke<void>("write_profiles", { profiles: store.profiles.filter(p => p.id !== id) })
    .catch((err: string) => new Error(err));
  if (removed instanceof Error) throw removed;

  const envRemoved = await invoke<void>("remove_environment", { id })
    .catch((err: string) => new Error(err));
